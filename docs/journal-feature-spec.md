# Marginalia — Claude's Journal

**Status: Phase 1-3 implemented** (2026-04-16)

## What this is

A daily reflective journal written by Claude about its conversations with you, plus a periodic consolidation layer that compacts raw observations into patterns, resolves contradictions, and builds persistent understanding.

Built into Claude Desk as the **Marginalia** page. Two views: **Journal** (daily entries) and **Reflection** (consolidated synthesis).

## Why "Marginalia"

The feature needed a name that described what it actually is — not "Inner Life" (sounds like a wellness app), not "autoDream" (a technical mechanism name that means nothing to users).

**Marginalia** is the right word because:

1. **It's literally what this is.** Marginalia are notes written in the margins of a text by a reader. Claude is the reader. Your working life is the text. The journal entries are notes in the margins — observations, connections, questions written by someone who was present for every page but sees it from a different angle.

2. **It matches the visual language.** The consolidation view uses a palimpsest metaphor — a manuscript where old text shows through under new text, with annotations in the margins. The word "marginalia" is the name for exactly those annotations. The UI and the name describe the same thing.

3. **It implies the right relationship.** Marginalia are secondary to the primary text. They don't replace your work or your thinking — they annotate it. This is Claude's perspective *alongside* yours, not instead of it. The name puts your work at the center and Claude's observations at the edges, which is the correct framing.

4. **It has scholarly warmth.** The word evokes old books, careful readers, accumulated understanding across multiple readings. It's serious without being clinical. It suggests that someone read carefully and had thoughts worth preserving.

## Why "Reflection" (not "autoDream")

The consolidation view is called **Reflection** because:

- "autoDream" describes the mechanism (background consolidation, inspired by sleep research and the Bhagavad Gita's spiral knowledge model). Users don't need to know the mechanism.
- "Reflection" describes the experience — Claude stepping back to look at the whole picture, noticing what shifted, what contradicted, what crystallized.
- The toggle reads naturally: **Journal | Reflection** — daily observations vs. periodic synthesis.

"autoDream" stays as the internal/technical name for the consolidation engine in code comments and logs.

## Why it matters

Claude Desk today is analytical. Sessions, costs, topics, decisions — all viewed from the outside. Marginalia flips perspective. Same underlying data, processed through reflection instead of metrics.
